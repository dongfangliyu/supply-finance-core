 ...
 */
public class BaseCompanyUserController {
     ...
     * 获取当前用户
     * */
    public CompanyUserEntity getCompanyUser(String userName){
        CompanyUserEntity user =redisService.get(userName,CompanyUserEntity.class);
        if(user!=null){
            return  user;
        }else{
            user = this.restTemplate.getForObject(userServicePath+"/company/user/findCompanyUserByName?userName={userName}",CompanyUserEntity.class,userName);
