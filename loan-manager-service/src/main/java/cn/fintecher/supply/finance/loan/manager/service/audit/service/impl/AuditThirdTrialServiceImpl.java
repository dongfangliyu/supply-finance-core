...
public class AuditThirdTrialServiceImpl implements AuditThirdTrialService {
    ...
    public Message receiveTask(SysUserAdminEntity user){
    	...
    	try {
    		//判断是否有审核权限
    		if (!IsExamineAuth(new Long(user.getUserId()), Constants.THIRD_ROLE_ID)) {
				 new Message<>(MessageType.MSG_ERROR,"audit","无领取权限");
			}
    		String userName = user.getUsername();
    		//查询可领取任务
			BusinessOrderEntity businessOrderEntity = new BusinessOrderEntity();
			businessOrderEntity.setProcessStatus(Constants.APPROVAL_THIRD_STAY);
			...
			for (int i = 0; i < listCanReceive.size(); i++) {
				...
				orderEntity.setProcessStatus(Constants.APPROVAL_THIRD_ALREADY); //改为终审已领取
				...
				infoEntity.setState(Constants.APPROVAL_THIRD_ALREADY);
    ...
    public Message searchOrderList(AuditOrderInfoFrom auditOrderInfoFrom){
    	...
    	try {
			...
			auditOrderInfoFrom.setNode(Constants.TASK_THIRD_NODE);
			Message<Integer> countMsg = auditOrderInfoService.selectThirdCount(auditOrderInfoFrom);
			Message<List<AuditOrderInfoEntity>> listMsg = auditOrderInfoService.selectThirdList(auditOrderInfoFrom);
    ...
    public Message submitContnet(AuditSumbitContentRequest auditSumbitContentRequest, SysUserAdminEntity user){
    	...
    	try {
    		...
    		if (!(auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_THIRD_REFUSE )||
    				...
    				auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_THIRD_ADOPT)||
    				auditSumbitContentRequest.getResult().equals( Constants.SUPP_GUAR_ADOPT)) ) {
    			return new Message<>(MessageType.MSG_ERROR,"audit",ReturnMsg.FAILED_CURRENCY);
			}
    		if (auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_THIRD_ADOPT)||
    				auditSumbitContentRequest.getResult().equals( Constants.SUPP_GUAR_ADOPT)) {
    		...
    		if (!Constants.APPROVAL_THIRD_ALREADY.equals(auditOrderInfo.getState())) {
    			//该条数据为非初审待审核数据，返回报错
    			return new Message<>(MessageType.MSG_ERROR,"audit",ReturnMsg.FAILED_CURRENCY);
			}
    		//判断当前用户是否能处理此数据
    		...
    		if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_THIRD_REFUSE)) {
    			auditTaskHistoryEntity.setResult("终审拒绝");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_THIRD_CANCEL)){
				auditTaskHistoryEntity.setResult("终审取消");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_THIRD_BACK)){
				auditTaskHistoryEntity.setResult("终审回退");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_THIRD_ADOPT)){
				auditTaskHistoryEntity.setResult("终审通过");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.SUPP_GUAR_ADOPT)){
	    		auditTaskHistoryEntity.setResult("终审通过");
	    	}
    		...
    		if (auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_THIRD_ADOPT)
    			...
    			if (auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_THIRD_ADOPT)) {
    				auditTaskHistoryEntity.setGuarantee(Constants.GUARANTEE_NEED);
    				orderInfo.setGuarantee(Constants.GUARANTEE_NEED);
				}else {
					auditTaskHistoryEntity.setGuarantee(Constants.GUARANTEE_NO_NEED);
					orderInfo.setGuarantee(Constants.GUARANTEE_NO_NEED);
				}
    		}
    		...
    		if (auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_THIRD_ADOPT)
        			||auditSumbitContentRequest.getResult().equals( Constants.SUPP_GUAR_ADOPT)) {
    			if (auditSumbitContentRequest.getResult().equals( Constants.SUPP_GUAR_ADOPT)) {
					order.setFinancingStatus("2");
				}
    			/*order.setApplicationAmount(auditSumbitContentRequest.getApprovalAmount());
    			order.setApplicationTerm(Integer.parseInt(auditSumbitContentRequest.getApprovalTerm()));*/
        		}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_FIRST_REFUSE )||
    		...
    		orderInfo.setThirdTrialTime(new Date());
    ...
    public Message searchDetailResult(Long pid){
    	...
    	try{
    		...
    		for (AuditTaskHistoryEntity auditTaskHistoryEntity : resultlist) {
				if ("终审".equals(auditTaskHistoryEntity.getNode())) {
