...
public class AuditFirstTrialServiceImpl implements AuditFirstTrialService {
	...
	public Message receiveTask(SysUserAdminEntity user){
		...
		try {
			...
			//修改order状态为已领取，添加order_info信息，添加task信息
			for (int i = 0; i < listCanReceive.size(); i++) {
				...
				orderEntity.setProcessStatus(Constants.APPROVAL_FIRST_ALREADY); //改为初审已领取
				...
				infoEntity.setEnterCategory(resultEntity.getEnterCategory());
				infoEntity.setEnterCategoryValue(resultEntity.getEnterCategoryValue());
				infoEntity.setSuppCategory(resultEntity.getSuppCategory());
				infoEntity.setSuppCategoryValue(resultEntity.getSuppCategoryValue());
				infoEntity.setAccountno(resultEntity.getAccountNo());
				infoEntity.setApplicationAmount(resultEntity.getApplicationAmount());
				infoEntity.setApplicationTerm(resultEntity.getApplicationTerm());
				infoEntity.setCertificateAmount(resultEntity.getCertificateAmount());
				infoEntity.setEnterpriseId(resultEntity.getEnterpriseId());
				infoEntity.setEnterpriseName(resultEntity.getEnterpriseName());
				infoEntity.setSubmitTime(resultEntity.getSubmitTime());
				//infoEntity.setFirstTrialTime(new Date()); //领取任务时不需要审核时间
				infoEntity.setOrderId(resultEntity.getPid());
				infoEntity.setContractNo(resultEntity.getContractNo());
				infoEntity.setStatus(Constants.DATA_STATUS_NOL);
				infoEntity.setSupplierId(resultEntity.getSupplierId());
				infoEntity.setSupplierName(resultEntity.getSupplierName());
				infoEntity.setState(Constants.APPROVAL_FIRST_ALREADY);
				infoEntity.setAccountStartTime(resultEntity.getAccountStartTime());
				infoEntity.setServiceFee(resultEntity.getServiceFee());
				infoEntity.setInterestRate(resultEntity.getInterestRate());
				infoEntity.setInterest(resultEntity.getInterest());
				infoEntity.setAccountConfirmTime(resultEntity.getAccountConfirmTime());
				infoEntity.setAccountEndTime(resultEntity.getAccountEndTime());
				infoEntity.setCreateBy(userName);
				infoEntity.setUpdateBy(userName);
				infoEntity.setCreateAt(new Date());
				infoEntity.setUpdateAt(new Date());
				Message infoMsg = auditOrderInfoService.insertOrderInfo(infoEntity);
				...
				taskEntity.setOrderInfoId(Long.valueOf(String.valueOf(infoMsg.getMessage())));
	...
	public Message searchOrderList(AuditOrderInfoFrom auditOrderInfoFrom){
		...
		try {
			auditOrderInfoFrom.setNode(Constants.TASK_FIRST_NODE);
			Message<Integer> countMsg = auditOrderInfoService.selectFristCount(auditOrderInfoFrom);
			Message<List<AuditOrderInfoEntity>> listMsg = auditOrderInfoService.selectFristList(auditOrderInfoFrom);
	...
	public Message submitContnet(AuditSumbitContentRequest auditSumbitContentRequest, SysUserAdminEntity user){
		...
		try {
			...
			if (!(auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_FIRST_REFUSE )||
					...
					auditSumbitContentRequest.getResult().equals( Constants.APPROVAL_FIRST_BACK)) ) {
				return new Message<>(MessageType.MSG_ERROR,"audit",ReturnMsg.FAILED_CURRENCY);
			}
			//判断该条数据是否为初审待处理数据
			...
			if (!Constants.APPROVAL_FIRST_ALREADY.equals(auditOrderInfo.getState())) {
				//该条数据为非初审待审核数据，返回报错
				return new Message<>(MessageType.MSG_ERROR,"audit",ReturnMsg.FAILED_CURRENCY);
			}
			//判断当前用户是否能处理此数据
			...
			if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_FIRST_REFUSE)) {
				auditTaskHistoryEntity.setResult("初审拒绝");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_FIRST_CANCEL)){
				auditTaskHistoryEntity.setResult("初审取消");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_FIRST_BACK)){
				auditTaskHistoryEntity.setResult("初审回退");
			}else if (auditSumbitContentRequest.getResult().equals(Constants.APPROVAL_SECOND_STAY)){
				auditTaskHistoryEntity.setResult("初审通过");
			}
			...
			orderInfo.setFirstTrialTime(new Date());
	...
	public Message searchDetailResult(Long pid){
		...
		try{
			...
			for (AuditTaskHistoryEntity auditTaskHistoryEntity : resultlist) {
				if ("初审".equals(auditTaskHistoryEntity.getNode())) {
	...
	private AuditFirstTrialResponse getAuditFirstTrialResponse(Long pid){
		...
		form.setId(pid.toString());
		form.setType("9");
		AuditResultEntity entity = fcAuditCompanyCore.searchSurveyOpinion(form);
		if(null!=entity){
			response.setAuditResultEntity(entity);
		}
		response.setFlag(1);
		return response;
	}
