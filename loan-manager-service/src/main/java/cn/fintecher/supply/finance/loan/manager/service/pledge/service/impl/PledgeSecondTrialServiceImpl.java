...
import cn.fintecher.supply.finance.loan.manager.service.pledge.service.PledgeSecondTrialService;
...
public class PledgeSecondTrialServiceImpl implements PledgeSecondTrialService {
	...
	public Message receiveTask(SysUserAdminEntity user) {
		...
		try {
			//判断是否有审核权限
			if (!IsExamineAuth(new Long(user.getUserId()), Constants.PLEDGE_SECOND_ROLE_ID)) {
				new Message<>(MessageType.MSG_ERROR,"pledge","无领取权限");
			}
			String userName = user.getUsername();
			//查询可领取任务 参数 1初审 2复审 3终审
			List<PledgeApplyStockInfoEntity> listSureReceive = pledgeApplyCore.sureReceiveTask(Constants.TASK_SECOND_NODE);
			...
			for (int i = 0; i < listSureReceive.size(); i++) {
				...
				updateEntity.setApplyRehearState(Constants.PLEDGE_TRIAL_ALREADY); //改为待审核
	...
	public Message searchPledgeList(PledgeStockTrialFrom pledgeStockTrialFrom, SysUserAdminEntity user) {
		...
		try {
			...
			pledgeStockTrialFrom.setNode(Constants.TASK_SECOND_NODE);
			//总数
	...
	public Message searchDetail(Long id, SysUserAdminEntity sysUserAdminEntity) {
		...
		for (AuditTaskHistoryEntity auditTask : historyList) {
			if ("复审".equals(auditTask.getNode())) {
	...
	public Message submitContnet(PledgeSumbitTrialRequest pledgeSumbitTrialRequest,
		...
		if (!Constants.PLEDGE_TRIAL_ALREADY.equals(applyStockInfo.getApplyRehearState())) {
		...
		if (pledgeSumbitTrialRequest.getResult().equals(Constants.PLEDGE_TRIAL_ADOPT)) {
			updateApplyStockInfo.setApplyFinalState(Constants.PLEDGE_TRIAL_STAY);
			...
			auditTaskHistoryEntity.setApplicationAmount(pledgeSumbitTrialRequest.getApprovalAmount().toString());
			...
			infoEntity.setContractApplyAmount(new BigDecimal(pledgeSumbitTrialRequest.getApprovalAmount()));
		...
		updateApplyStockInfo.setApplyRehearState(pledgeSumbitTrialRequest.getResult());
		...
		updateApplyStockInfo.setApplyRehearTime(new Date());
