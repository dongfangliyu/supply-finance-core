...
@Api(tags = "核心企业签约管理")
public class AuditEnterSigningController {
    @Autowired
     ...
     * 核心企业签约管理列表
     */
    @ApiOperation(value = "核心企业签约管理列表", notes = "核心企业签约管理列表")
     ...
     * 核心企业签约详情
     */
    @ApiOperation(value = "核心企业签约详情", notes = "核心企业签约详情")
    @ApiImplicitParam(name = "Authorization", required = false, dataType = "string", paramType = "header")
    ...
    public Message submitSigning(@RequestParam("pid") Long pid,Principal principal){
        try {
            return auditEnterSigningService.submitSigning(pid,principal.getName());
        }catch (Exception e){
            e.printStackTrace();
            return new Message(MessageType.MSG_ERROR,"audit_bff",e.getMessage());
        }
