...
@Api(tags = "供应商签约管理")
public class AuditSuppSigningController {
    ...
    public Message isUpLoadContract(@RequestParam("pid") Long pid){
        try {
            return auditSuppSigningService.isUpLoadContract(pid);
        }catch (Exception e){
            e.printStackTrace();
     ...
     * 供应商签约管理列表
     */
    @ApiOperation(value = "供应商签约管理列表", notes = "供应商签约管理列表")
    ...
    public Message searchSigningList(@RequestBody AuditSigningRequest auditSigningRequest, Principal principal){
        try {
            auditSigningRequest.setCurrentUserName(principal.getName());
            return auditSuppSigningService.searchSigningList(auditSigningRequest);
        }catch (Exception e){
            e.printStackTrace();
            return new Message(MessageType.MSG_ERROR,"audit_bff",e.getMessage());
        }
     ...
     * 供应商签约详情
     */
    @ApiOperation(value = "供应商签约详情", notes = "供应商签约详情")
    @ApiImplicitParam(name = "Authorization", required = false, dataType = "string", paramType = "header")
     ...
     * 保存还款账户
     */
    @ApiOperation(value = "保存还款账户", notes = "保存还款账户")
    @ApiImplicitParam(name = "Authorization", required = true, dataType = "string", paramType = "header")
    @RequestMapping(value = "/submitSigningAccount", method = RequestMethod.POST)
    public Message submitSigningAccount(@RequestBody AuditRepayBankInfoForm auditRepayBankInfoForm,Principal principal){
        try {
            auditRepayBankInfoForm.setCurrentUserName(principal.getName());
            return auditSuppSigningService.submitSigningAccount(auditRepayBankInfoForm);
        }catch (Exception e){
            e.printStackTrace();
            return new Message(MessageType.MSG_ERROR,"audit_bff",e.getMessage());
        }
    ...
    public Message submitSigning(@RequestParam("pid") Long pid,Principal principal){
        try {
            return auditSuppSigningService.submitSigning(pid,principal.getName());
        }catch (Exception e){
            e.printStackTrace();
            return new Message(MessageType.MSG_ERROR,"audit_bff",e.getMessage());
        }
